...
/**
  ...
  */
private[akka] abstract class AkkaActorIntermediator[F[_]: Timer](
    name: String,
    selection: ActorRefSelection[F],
    askTimeout: Timeout
)(implicit F: Effect[F], as: ActorSystem, policy: RetryPolicy[F]) {

  private[akka] implicit val contextShift: ContextShift[IO]        = IO.contextShift(as.dispatcher)
  private[akka] implicit def noop[A]: (A, RetryDetails) => F[Unit] = retry.noop[F, A]
